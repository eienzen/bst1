...
const abi = [
    ...
    // ... (rest of the ABI)
    // Include all other ABI entries here
];
...
async function updatePlayerInfo() {
    ...
    try {
        ...
        rewardList.innerHTML = rewards.map(r => `<li>${r.amount} BST at ${new Date(r.timestamp * 1000).toLocaleString()}</li>`).join("");
    } catch (error) {
...
document.getElementById("convertPoints").addEventListener("click", async () => {
    ...
    try {
        ...
        await contract.convertPointsToTokens(amount, account, referrer, { value: ethers.utils.parseEther("0.0002"), gasLimit: 300000 });
...
document.getElementById("stakeTokens").addEventListener("click", async () => {
    ...
    try {
        ...
        await contract.stakeTokens(amount, lockPeriod, { gasLimit: 300000 });
        alert("Tokens staked!");
        updatePlayerInfo();
    } catch (error) {
...
document.getElementById("unstakeTokens").addEventListener("click", async () => {
    ...
    try {
        ...
        await contract.unstakeTokens(amount, lockPeriod, { gasLimit: 300000 });
